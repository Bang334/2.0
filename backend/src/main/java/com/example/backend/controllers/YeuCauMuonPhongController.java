...
public class YeuCauMuonPhongController {
    ...
    public ResponseEntity<?> timPhongTrong(
            ...
            @RequestParam(required = false) List<String> thietBiYeuCau) {
        try {
            ...
            // Lọc theo thiết bị yêu cầu nếu có
            if (thietBiYeuCau != null && !thietBiYeuCau.isEmpty()) {
                // Tạo danh sách phòng tạm để lọc
                List<Phong> filteredPhongByEquipment = new ArrayList<>();
                
                for (Phong phong : danhSachPhong) {
                    boolean hasAllRequiredEquipment = true;
                    Set<PhongThietBi> phongThietBiSet = phong.getThietBis();
                    
                    // Kiểm tra từng thiết bị trong danh sách yêu cầu
                    for (String thietBiId : thietBiYeuCau) {
                        boolean found = false;
                        for (PhongThietBi phongThietBi : phongThietBiSet) {
                            // Kiểm tra xem thiết bị có phải là thietBiId không
                            // Chuyển về String để phù hợp với các ID từ frontend
                            String equipmentValue = phongThietBi.getThietBi().getMaThietBi().toString();
                            if (equipmentValue.equals(thietBiId) || 
                                phongThietBi.getThietBi().getTenThietBi().toLowerCase().contains(thietBiId.toLowerCase())) {
                                found = true;
                                break;
                            }
                        }
                        ...
                        if (!found) {
                            hasAllRequiredEquipment = false;
                            break;
                        }
                    }
                    
                    if (hasAllRequiredEquipment) {
                        filteredPhongByEquipment.add(phong);
                    }
                }
                
                // Cập nhật danh sách phòng với kết quả đã lọc theo thiết bị
                danhSachPhong = filteredPhongByEquipment;
            }
