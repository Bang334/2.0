...
public class PhanCongMuonPhongController {
    ...
    private List<PhanCongMuonPhongResponse> chuyenDoiDanhSachPhanCong(List<PhanCongMuonPhong> danhSachPhanCong) {
        ...
        for (PhanCongMuonPhong phanCong : danhSachPhanCong) {
            ...
            response.setTenMonHoc(tkb.getMonHoc().getTenMon());
            response.setMaPhong(tkb.getPhong().getMaPhong());
            
            // Tính toán thời gian bắt đầu và kết thúc từ tiết học
            Date thoiGianBatDau = tinhThoiGianBatDau(tkb.getTietBatDau(), tkb.getNgayHoc());
            response.setThoiGianBatDau(thoiGianBatDau);
            
            Date thoiGianKetThuc = tinhThoiGianKetThuc(tkb.getTietKetThuc(), tkb.getNgayHoc());
            response.setThoiGianKetThuc(thoiGianKetThuc);
