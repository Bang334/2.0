...
public class PhanCongMuonPhongController {
    ...
    /**
     * Lấy tất cả danh sách phân công trong lớp (dành cho mọi sinh viên)
     */
    @GetMapping("/tatca")
    @PreAuthorize("hasRole('SV')")
    public ResponseEntity<?> layTatCaDanhSachPhanCong() {
        try {
            UserDetailsImpl userDetails = (UserDetailsImpl) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
            String userId = userDetails.getId();
            ...
            if (sinhVien.getLopHoc() == null) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                        .body(new MessageResponse("Không tìm thấy thông tin lớp học của sinh viên"));
            }
            
            // Lấy danh sách phân công trong lớp
            List<PhanCongMuonPhong> danhSachPhanCong = phanCongMuonPhongService.layTatCaDanhSachPhanCongTrongLop(sinhVien.getLopHoc().getMaLop());
            List<PhanCongMuonPhongResponse> responseList = chuyenDoiDanhSachPhanCong(danhSachPhanCong);
    ...
    public ResponseEntity<?> capNhatTrangThaiPhanCong(
        ...
        try {
            ...
            // Kiểm tra nếu là từ chối thì cần lý do
            if (trangThai == PhanCongMuonPhong.TrangThai.TUCHOI && (lyDo == null || lyDo.trim().isEmpty())) {
                // Nếu không có lý do thì vẫn cho phép, chỉ log cảnh báo
                System.out.println("CẢNH BÁO: Từ chối phân công không có lý do");
            }
            
            // Cập nhật trạng thái
            PhanCongMuonPhong phanCong = phanCongMuonPhongService.capNhatTrangThai(maPhanCong, trangThai, lyDo);
    ...
    private List<PhanCongMuonPhongResponse> chuyenDoiDanhSachPhanCong(List<PhanCongMuonPhong> danhSachPhanCong) {
        ...
        for (PhanCongMuonPhong phanCong : danhSachPhanCong) {
            ...
            response.setLyDoTuChoi(phanCong.getLyDoTuChoi());
