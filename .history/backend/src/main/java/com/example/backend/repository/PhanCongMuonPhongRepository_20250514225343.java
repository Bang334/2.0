...
public interface PhanCongMuonPhongRepository extends JpaRepository<PhanCongMuonPhong, Integer> {
    ...
    @Query("SELECT pc FROM PhanCongMuonPhong pc WHERE pc.nguoiDuocPhanCong.maSV = :maSV AND pc.trangThai = 'CHUAXULY'")
    ...
    // Lấy tất cả phân công của sinh viên trong một lớp
    @Query("SELECT pc FROM PhanCongMuonPhong pc " +
           "WHERE pc.nguoiPhanCong.lopHoc.maLop = :maLop OR pc.nguoiDuocPhanCong.lopHoc.maLop = :maLop " +
           "ORDER BY pc.thoiGianPhanCong DESC")
    List<PhanCongMuonPhong> findAllAssignmentsInClass(@Param("maLop") String maLop);
} 