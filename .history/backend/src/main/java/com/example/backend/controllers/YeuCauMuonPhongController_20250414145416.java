            ...
            for (Phong phong : danhSachPhong) {
                ...
                for (Map<String, Date> busy : busyIntervals) {
                    ...
                    if (lastEndTime.before(busy.get("start"))) {
                        ...
                        if (busy.get("start").after(currentTime)) {
                            ...
                            System.out.println("Thêm khoảng trống: " + khoang.get("start") + " đến " + khoang.get("end"));
                            khoangThoiGianTrong.add(khoang);
                        }
                    }
                    lastEndTime = busy.get("end");
                    System.out.println("Cập nhật lastEndTime = " + lastEndTime);
                }
                ...
                if (lastEndTime.before(endOfDay)) {
                    ...
                    if (endOfDay.after(currentTime)) {
                        ...
                        System.out.println("Thêm khoảng trống cuối ngày: " + khoang.get("start") + " đến " + khoang.get("end"));
                        khoangThoiGianTrong.add(khoang);
                    }
                }

                // Sửa: Hiển thị rõ tất cả các khoảng thời gian trống đã tìm thấy
                System.out.println("Tất cả khoảng thời gian trống cho phòng " + phong.getMaPhong() + ":");
                for (Map<String, Date> khoang : khoangThoiGianTrong) {
                    System.out.println("  Có thể mượn từ: " + khoang.get("start") + " đến " + khoang.get("end"));
                }
                ...
                for (Map<String, Date> khoang : khoangThoiGianTrong) {
                    ...
                    if (end.getTime() - start.getTime() >= duration) {
                        // Tạo các khoảng thời gian có thể
                        List<Map<String, Date>> possibleSlots = new ArrayList<>();
                        
                        // Tạo một khoảng thời gian đầu tiên (thời điểm bắt đầu khoảng trống)
                        if (end.getTime() - start.getTime() >= duration) {
                            Map<String, Date> slot = new HashMap<>();
                            slot.put("start", start);
                            slot.put("end", new Date(start.getTime() + duration));
                            possibleSlots.add(slot);
                        }
                        
                        // Tạo một khoảng thời gian cuối (kết thúc ở cuối khoảng trống)
                        if (end.getTime() - start.getTime() >= duration) {
                            Map<String, Date> slot = new HashMap<>();
                            slot.put("start", new Date(end.getTime() - duration));
                            slot.put("end", end);
                            possibleSlots.add(slot);
                        }
                        
                        // Tạo một khoảng thời gian ở giữa nếu khoảng trống đủ lớn
                        if (end.getTime() - start.getTime() > duration) {
                            long midpoint = start.getTime() + (end.getTime() - start.getTime()) / 2;
                            Map<String, Date> slot = new HashMap<>();
                            slot.put("start", new Date(midpoint - duration / 2));
                            slot.put("end", new Date(midpoint + duration / 2));
                            possibleSlots.add(slot);
                        }
                        
                        // Kiểm tra từng khoảng thời gian có thể để tìm khoảng gần nhất
                        for (Map<String, Date> slot : possibleSlots) {
                            Date goiYStart = slot.get("start");
                            Date goiYEnd = slot.get("end");
                            boolean isConflict = false;
                            ...
                            if (!isConflict) {
                                // Tính khoảng cách thời gian giữa khoảng đề xuất và khoảng yêu cầu
                                // Sử dụng điểm giữa của khoảng thời gian để tính khoảng cách
                                long requestMidpoint = thoiGianMuon.getTime() + (thoiGianTra.getTime() - thoiGianMuon.getTime()) / 2;
                                long slotMidpoint = goiYStart.getTime() + (goiYEnd.getTime() - goiYStart.getTime()) / 2;
                                long diff = Math.abs(slotMidpoint - requestMidpoint);
                                
                                System.out.println("Khoảng thời gian " + goiYStart + " đến " + goiYEnd + 
                                                 " có độ lệch " + diff + " so với yêu cầu " + 
                                                 thoiGianMuon + " đến " + thoiGianTra);
