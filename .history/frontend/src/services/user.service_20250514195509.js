...
class UserService {
  ...
  layDanhSachPhanCongChoSinhVien = () => {
    return axios.get(API_URL + "phancong/sinhvien", { headers: authHeader() });
  };

  layDanhSachPhanCongCuaCanBo = () => {
    return axios.get(API_URL + "phancong/canbo", { headers: authHeader() });
  };

  taoPhanCong = (maSVNguoiDuocPhanCong, maTKB, ghiChu) => {
    return axios.post(API_URL + "phancong", {
  ...
  huyPhanCong = (maPhanCong) => {
    return axios.delete(API_URL + `phancong/${maPhanCong}`, { headers: authHeader() });
  };

  capNhatTrangThaiPhanCong = (maPhanCong, trangThai) => {
    return axios.put(API_URL + `phancong/${maPhanCong}/trangthai?trangThai=${trangThai}`, {}, { headers: authHeader() });
  };
}
