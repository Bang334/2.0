      ...
      return this.getDanhSachLopHocGV()
        .then(response => {
          if (response.data && response.data.length > 0) {
            ...
            console.log("Getting students for class:", firstClass);
            return this.getDanhSachSinhVienLop(firstClass.maLop);
          }
          console.log("No classes found for instructor");
          return { data: [] };
        })
        .catch(error => {
          console.error("Error getting instructor's classes:", error);
          throw error;
        });
    } else {
