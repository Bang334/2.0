...
class UserService {
  ...
  getLichHocChoThoiKhoaBieu() {
    ...
    // Gọi API để lấy tất cả thời khóa biểu
    return api.get('/thoikhoabieu/lop', { headers: headers })
      .then(response => {
        // Lọc ra chỉ những thời khóa biểu trong tương lai
        if (response.data && Array.isArray(response.data)) {
          const filteredData = response.data.filter(tkb => {
            // Chuyển ngày của thời khóa biểu sang Date object
            const tkbDate = new Date(tkb.ngayHoc);
            // Chỉ giữ lại những thời khóa biểu có ngày >= ngày hiện tại
            return tkbDate >= today;
          });
          
          console.log(`Đã lọc ${filteredData.length}/${response.data.length} thời khóa biểu trong tương lai`);
