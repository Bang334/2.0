...
class UserService {
  ...
  layDanhSachPhanCongCuaCanBo() {
    return api.get("/sinhvien/phancong/daghep", { headers: authHeader() });
  }

  layDanhSachPhanCongChoSinhVien() {
    return api.get("/sinhvien/phancong/nhan", { headers: authHeader() });
  }

  taoPhanCong(maSVNguoiDuocPhanCong, maTKB, ghiChu) {
    return api.post(
      "/sinhvien/phancong/tao", 
      { 
  ...
  huyPhanCong(maPhanCong) {
    return api.delete(`/sinhvien/phancong/${maPhanCong}`, { headers: authHeader() });
  }

  capNhatTrangThaiPhanCong(maPhanCong, trangThai) {
    return api.put(
      `/sinhvien/phancong/${maPhanCong}/trangthai`, 
      { trangThai }, 
      { headers: authHeader() }
    );
  }
