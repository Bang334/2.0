...
const PhanCongMuonPhongManager = () => {
  ...
  // Lấy danh sách phân công từ API
  const fetchDanhSachPhanCong = async () => {
    setLoading(true);
    try {
      const response = await api.get("/sinhvien/phancong");
      if (response.data) {
  ...
  const fetchDanhSachSinhVien = async () => {
    try {
      const response = await api.get("/sinhvien/danhsach-lop");
      if (response.data) {
        setDanhSachSinhVien(response.data);
      }
    } catch (error) {
  ...
  const loadStudentStats = async () => {
    ...
    try {
      // Đảm bảo có danh sách sinh viên trước
      if (danhSachSinhVien.length === 0) {
        await fetchDanhSachSinhVien();
      }

      // Đảm bảo có danh sách phân công
      if (danhSachPhanCong.length === 0) {
        await fetchDanhSachPhanCong();
      }
      ...
      danhSachPhanCong.forEach(assignment => {
