...
const PhanCongMuonPhongManager = () => {
    ...
    const checkCanBoRole = async () => {
      try {
        if (vaiTro === "SV") {
          ...
          // Chỉ fetch dữ liệu sau khi đã cập nhật trạng thái isCanBo
          fetchData(canBoStatus);
        } else {
          fetchData(false);
        }
      } catch (error) {
  ...
  const fetchData = async (canBoStatus = isCanBo) => {
    setLoading(true);
    try {
      if (activeTab === "phanCong") {
        // Chỉ gọi API nếu SV là cán bộ lớp hoặc user là giảng viên
        if (canBoStatus || vaiTro === "GV") {
          const response = await UserService.layDanhSachPhanCongCuaCanBo();
          setDanhSachPhanCong(response.data);
          setError(""); // Xóa lỗi nếu trước đó đã hiển thị
        } else {
          setDanhSachPhanCong([]);
          setError("Bạn không có quyền xem danh sách phân công. Chỉ cán bộ lớp mới có quyền này.");
        }
      } else if (activeTab === "nhanPhanCong") {
  ...
  const handleViewDetail = async (maPhanCong, isOwn = true) => {
    ...
    try {
      ...
      const phanCong = isOwn 
        ...
        : danhSachPhanCongNhan.find(item => item.maPhanCong === maPhanCong);
          ...
          <Card>
            <Card.Header className="d-flex justify-content-between align-items-center">
              ...
              <div className="d-flex">
                {/* Para cán bộ lớp, mostrar solo la pestaña de asignación */}
                {isCanBo ? (
                  <div className="d-flex">
                  ...
                  // Para estudiantes normales, mostrar pestañas para ver asignaciones
                  <div className="d-flex">
                    <Button
            ...
            <Card.Body>
              ...
              {loading ? (
                ...
                // Vista para cán bộ lớp - Solo muestra la tabla de asignaciones que ha creado
                        ...
                        <tr key={item.maPhanCong}>
                          ...
                          <td>
                            <Button
                              ...
                              size="sm"
                              className="me-1"
                              onClick={() => handleViewDetail(item.maPhanCong, true)}
                              disabled={loading}
                            >
                              <FaEye />
                            </Button>
                ...
                // Vista para estudiantes normales - Pestaña "Phân công"
                <Alert variant="info">
                  Chức năng này chỉ dành cho cán bộ lớp. Vui lòng xem các phân công được nhận ở tab "Phân công được nhận".
                </Alert>
              ) : (
                // Vista para estudiantes normales - Pestaña "Phân công được nhận"
                <Table striped bordered hover responsive>
                  <thead>
                    <tr>
                        ...
                        <tr key={item.maPhanCong}>
                          ...
                          <td>
                            <Button
                              ...
                              size="sm"
                              className="me-1"
                              onClick={() => handleViewDetail(item.maPhanCong, false)}
                              disabled={loading}
                            >
                              <FaEye />
                            </Button>
