...
const PhanCongMuonPhongManager = () => {
  ...
  // Thêm useEffect để theo dõi thay đổi trong danhSachPhanCong
  useEffect(() => {
    if (vaiTro === "SV") {
      // Đếm số lượng phân công chưa xử lý dành cho sinh viên hiện tại
      const pendingCount = danhSachPhanCong.filter(
        item => item.isForMe && item.trangThai === "CHUAXULY"
      ).length;

      // Gửi event để cập nhật badge
      const event = new CustomEvent('pendingAssignmentsUpdated', { detail: pendingCount });
