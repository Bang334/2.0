...
const PhanCongMuonPhongManager = () => {
  ...
  const openModalPhanCong = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/sinhvien/danhsach-lop");
      const data = await response.json();
      setDanhSachSinhVien(data);

      // Fetch danh sách thời khóa biểu
      setLoadingTKB(true);
      const responseTKB = await fetch("/api/thoikhoabieu/lop");
      const dataTKB = await responseTKB.json();
      setDanhSachTKB(dataTKB);
    } catch (error) {
