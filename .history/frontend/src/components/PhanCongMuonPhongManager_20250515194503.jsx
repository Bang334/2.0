...
const PhanCongMuonPhongManager = () => {
  ...
  // Thêm hàm để export số lượng phân công chưa xử lý
  window.getPendingAssignmentsCount = () => {
    return danhSachPhanCong.filter(
      item => item.isForMe && item.trangThai === "CHUAXULY"
    ).length;
  };

  useEffect(() => {
    // Cập nhật số lượng phân công chưa xử lý mỗi khi danh sách thay đổi
    const updatePendingCount = () => {
      if (!vaiTro || vaiTro !== "SV") return;
      const count = danhSachPhanCong.filter(
        item => item.isForMe && item.trangThai === "CHUAXULY"
      ).length;
      setPendingAssignments(count);
      // Trigger một custom event để thông báo cho navbar
      const event = new CustomEvent('pendingAssignmentsUpdated', { detail: count });
      window.dispatchEvent(event);
    };

    updatePendingCount();
  }, [danhSachPhanCong, vaiTro]);
