    ...
    const checkCanBoRole = async () => {
      try {
        if (vaiTro === "SV") {
          ...
          setIsCanBo(response.data.isCanBo);
          console.log("Is Cán Bộ Lớp:", response.data.isCanBo);
        }
      } catch (error) {
  ...
  const fetchData = async () => {
    ...
    try {
      if (activeTab === "phanCong") {
        // Chỉ gọi API này nếu SV là cán bộ lớp hoặc user là giảng viên
        if (isCanBo || vaiTro === "GV") {
