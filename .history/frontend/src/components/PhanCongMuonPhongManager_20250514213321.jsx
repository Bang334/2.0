...
const PhanCongMuonPhongManager = () => {
  ...
  const fetchAllAssignments = async (canBoStatus = isCanBo) => {
    ...
    try {
      ...
      // 3. Kết hợp và sắp xếp theo thời gian (mới nhất lên đầu)
      const allAssignments = [...canBoAssignments, ...receivedAssignments]
        .sort((a, b) => new Date(b.thoiGianPhanCong) - new Date(a.thoiGianPhanCong));
